...
test("gallery uses matched scene backdrops and lazy-loads secondary artworks", async ({ page }) => {
  ...
  await expect(page.getByTestId("gallery-route-stop").first().locator("img")).toHaveAttribute("loading", "eager");
});
...
test("gallery cards fall back when artwork images fail to load", async ({ page }) => {
  ...
  await gotoApp(page, "/artwork/met-438417?query=I+want+a+quiet+moonlit+room");
