...
test("detail page renders explanation slot with generated ready content", async ({ page }) => {
  ...
  await expect(page.getByTestId("explanation-evidence")).toContainText("Grounded evidence");
  await expect(page.getByTestId("explanation-evidence")).toContainText("Score:");
