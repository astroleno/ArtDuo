...
test("real image provider smoke embeds one PNG and one JPEG", {
  ...
  assert.equal(provenance.artifact, IMAGE_MODEL_ARTIFACT);
  assert.equal(provenance.checksum, IMAGE_MODEL_ARTIFACT_SHA256);
  assert.equal(provenance.providerVersion, IMAGE_PROVIDER_VERSION);
  assert.equal(embedded.every((entry) => entry.modelArtifactChecksum === provenance.checksum), true);
  assert.equal(embedded.every((entry) => entry.providerVersion === provenance.providerVersion), true);
