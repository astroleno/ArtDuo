 ...
 * LLM生成的策展意图接口
 */
export interface LLMCurationIntent {
 ...
 * LLM策展意图生成器
 */
export class LLMCurationIntentGenerator {
   ...
   * 生成策展意图
   */
  async generateCurationIntent(userInput: string): Promise<LLMCurationIntent> {
    console.log('🧠 开始LLM生成策展意图...');
    ...
    try {
      ...
      const response = await this.client.chat({
        ...
        messages: [
          {
            role: 'system',
            content: '你是一位专业的艺术策展人，擅长分析用户的情感需求并制定个性化的策展方案。'
          },
          {
            role: 'user',
            content: prompt
...
请以JSON格式输出，确保结构完整。`;
   ...
   * 解析LLM响应
   */
  private parseLLMResponse(content: string): LLMCurationIntent {
    try {
      // 尝试直接解析JSON
   ...
   * 验证和标准化策展意图
   */
  private validateAndNormalizeIntent(intent: any): LLMCurationIntent {
    return {
   ...
   * 从文本中提取策展意图
   */
  private extractIntentFromText(content: string): LLMCurationIntent {
    // 简单的文本解析逻辑
   ...
   * 获取默认策展意图
   */
  private getDefaultCurationIntent(): LLMCurationIntent {
    return {
      emotionalStages: [
        {
          stage: 1,
 ...
 * 批量生成策展意图
 */
export async function batchGenerateCurationIntents(
  userInputs: string[]
): Promise<LLMCurationIntent[]> {
  const generator = new LLMCurationIntentGenerator();
  const results: LLMCurationIntent[] = [];

  for (const userInput of userInputs) {
    try {
      const intent = await generator.generateCurationIntent(userInput);
      results.push(intent);
    } catch (error) {
      console.error(`❌ 生成策展意图失败: ${userInput}`, error);
      results.push(generator.getDefaultCurationIntent());
    }
  }
 ...
 * 策展意图质量评估
 */
export function evaluateCurationIntentQuality(intent: LLMCurationIntent): {
  score: number;
  details: string[];
} {
  let score = 0;
  ...
  // 评估审美偏好
  if (intent.aestheticPreferences.mediaTypes.length > 0) {
    score += 15;
    details.push('媒介类型明确');
  }
  if (intent.aestheticPreferences.artStyles.length > 0) {
    score += 15;
    details.push('艺术风格明确');
  }
...
// 导出单例实例
export const llmCurationIntentGenerator = new LLMCurationIntentGenerator();
