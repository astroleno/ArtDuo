...
const CHINESE_RESISTANCE_TARGETS: Array<{
  pattern: RegExp;
  signals: string[];
  highIntensitySignals?: string[];
}> = [
  { pattern: /(?:明亮|亮|bright)/u, signals: ["bright"] },
  { pattern: /(?:吵闹|热闹|喧闹|吵)/u, signals: ["loud"] },
  {
    pattern: /(?:悲伤|哀伤|忧伤)/u,
    signals: ["sadness"],
    highIntensitySignals: ["heavy-grief"],
  },
  { pattern: /(?:绝望|沉重|悲恸)/u, signals: ["heavy-grief"] },
  { pattern: /(?:剧情|戏剧性|戏剧)/u, signals: ["heavy-drama"] },
];

const CHINESE_DIRECT_NEGATION_TARGETS: Array<[RegExp, string[]]> = [
  [/^(?:悲伤|哀伤|忧伤)/u, ["sadness"]],
  [/^(?:绝望|沉重|悲恸)/u, ["heavy-grief"]],
];

const CHINESE_RESISTANCE_OPERATOR = /不要|不想|不能|避免|拒绝|别/gu;
const CHINESE_DIRECT_NEGATION_OPERATOR = /不(?=悲伤|哀伤|忧伤|绝望|沉重|悲恸)/gu;
const CHINESE_NEGATED_DIRECT_RESISTANCE_SCOPE = /(?:不得|不是|不能|不要|不想|不希望|不愿意|不愿|避免|拒绝|别)$/u;
const CHINESE_BIE_COMPOUND_PREFIX = /[个区差类识特分性级]/u;
const CHINESE_HIGH_INTENSITY_MODIFIER = /(?:那么多|太多|太过|过分|过于|过度|很|非常|那么|太|特别|格外|极其|十分|相当|颇为)/u;
const CHINESE_DIRECT_OBJECT_PREFIX = /^(?:(?:请|只|都|再|要|有|那么多|太多|很|非常|那么|太过|过分|过于|过度|太|特别|格外|极其|十分|相当|颇为|有点|稍微|略微|任何|一切|所有|这种|那种|这些|那些|有些|一些|一点点|一点|给我|给人|给|带有|带来|带|包含|充满|呈现|出现|显得|看到|看见|观看|看|接受|面对|感受|接触|选择|让我|让人|使我|令人|的))*$/u;
...
function operatorIsLexicallyIndependent(sourceText: string, operator: string, operatorIndex: number): boolean {
  if (operator !== "别" || operatorIndex === 0) {
    return true;
  }
  return !CHINESE_BIE_COMPOUND_PREFIX.test(sourceText[operatorIndex - 1] ?? "");
}

function directNegationIsNegated(sourceText: string, operatorIndex: number): boolean {
  const prefix = sourceText.slice(0, operatorIndex).trimEnd();
  return CHINESE_NEGATED_DIRECT_RESISTANCE_SCOPE.test(prefix);
}

function directObjectPrefix(sourceText: string, startIndex: number, target: RegExp): string | undefined {
  ...
  return CHINESE_DIRECT_OBJECT_PREFIX.test(prefix) ? prefix : undefined;
}

function directTargetSignals(
  sourceText: string,
  startIndex: number,
  target: (typeof CHINESE_RESISTANCE_TARGETS)[number],
): string[] {
  const prefix = directObjectPrefix(sourceText, startIndex, target.pattern);
  if (prefix === undefined) {
    return [];
  }
  if (target.highIntensitySignals && CHINESE_HIGH_INTENSITY_MODIFIER.test(prefix)) {
    return target.highIntensitySignals;
  }
...
function collectChineseResistances(sourceText: string): string[] {
  ...
  for (const match of sourceText.matchAll(CHINESE_DIRECT_NEGATION_OPERATOR)) {
    const operatorIndex = match.index;
    if (operatorIndex === undefined || directNegationIsNegated(sourceText, operatorIndex)) {
      continue;
    }
    const targetText = sourceText.slice(operatorIndex + match[0].length);
    values.push(
      ...CHINESE_DIRECT_NEGATION_TARGETS
        .flatMap(([pattern, signals]) => pattern.test(targetText) ? signals : []),
    );
  }
  ...
  for (const match of sourceText.matchAll(CHINESE_RESISTANCE_OPERATOR)) {
    ...
    if (
      ...
      || !operatorIsLexicallyIndependent(sourceText, operator, operatorIndex)
      || operatorIsNegated(sourceText, operator, operatorIndex)
    ) {
      continue;
    }
    values.push(
      ...CHINESE_RESISTANCE_TARGETS
        .flatMap((target) => directTargetSignals(sourceText, operatorIndex + operator.length, target)),
    );
  }
