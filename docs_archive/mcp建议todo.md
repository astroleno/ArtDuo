...
### MCP + LLM 串接 Checklist (MVP)

- [ ] **环境与配置准备**
  - [ ] 明确 MCP 上游地址、认证方式、超时阈值，并写入 `.env.local`/运行时配置
  - [ ] 确认 LLM 提供方（OpenAI/Azure/本地服务）与密钥管理方案，配置 `LLM_MODEL`、`LLM_ENDPOINT`
  - [ ] 为开发/生产分别设置 `NEXT_PUBLIC_MCP_MOCK_ENABLED` 等开关，确保可切换真实链路

- [ ] **LLM 调用链路** (`frontend/src/lib/frontend-agent.ts`)
  - [ ] 定义触发 LLM 的状态流转：接收前端输入后先进入 `analysis:loading`
  - [ ] 组装 Prompt（包含情绪、场景、格式约束）并调用 LLM；记录 requestId/seed 方便追踪
  - [ ] 对 LLM 返回结果做结构校验（空值、字段缺失、JSON 解析），失败时返回 `ANALYSIS_EMPTY`
  - [ ] 成功后生成检索 query payload，写入 `generateAnalysis` 的输出结构

- [ ] **MCP 请求链路** (`frontend/src/app/api/mcp/route.ts` + `frontend/src/lib/frontend-mcp-client.ts`)
  - [ ] 在 API Route 中读取 LLM 输出的 query，向 MCP 服务发起真实请求
  - [ ] 设置超时/重试策略，捕获上游错误并按 `502/503 + code` 返回给前端
  - [ ] 在客户端封装里解析响应体，使用 `ArtworkSchema.safeParse` 校验数据
  - [ ] 将失败状态透传至 UI，不做本地 mock 或兜底数据

- [ ] **前端状态与 UI**
  - [ ] 在 `frontend/src/lib/frontend-agent.ts` 中串联 LLM→MCP→渲染的状态流（loading/success/error）
  - [ ] 去重、打标签、补充 `museum/objectUrl/source` 等字段后写入展线状态
  - [ ] 在 `frontend/src/components/ArtworkCard.tsx` 等组件中展示溯源信息，并对错误场景给出提示

- [ ] **监控与可观测性**
  - [ ] 记录每次调用的 requestId、LLM latency、MCP latency、错误码，便于调试
  - [ ] 为核心链路添加日志/埋点，确保上线后可追踪故障

- [ ] **联调验收**
  - [ ] 在开发环境走通真实链路：同一输入可复现 LLM 分析 + MCP 数据
  - [ ] 覆盖断网、超时、空分析、MCP schema 错误等主要失败路径，确认 UI 反馈正确
