...
### 1) 讲解（explanations）提示词与生成方式（已完成✅，已测试）
...
- 并发与限流：每批 3 条、批内并发 ≤3、抖动 30–80ms、批间 120ms；对 429/timeout 仅该条 1s/2s/4s 退避重试。（已实现并验证）
- 片段化（可选下一步）：五字段改为五个“小段请求”，SSE 流式逐条返回；失败片段单独回退模板并标记 degraded。
- 缓存：key = artworkId+emotion+hash(userInput)+field，TTL 24h；命中立即返回 fromCache。（已实现，默认关闭，开关 EXPLAIN_CACHE_ENABLED）

### 2) 评分（scoring）提示词与解析（已完成✅，已测试）
- 批量评分返回统一 schema：
...
- 解析：zod 校验 → 失败回退模板分数并标记 degraded。（已实现并验证）
- 并发控制：批次化 + 指数退避；仅在必要时缩短 max_tokens/temperature 以稳态快速完成。（已实现并验证）
...
### 6) 流式输出（SSE）与增量渲染（已完成✅，SSE已实现并通过测试；目前默认关闭，可按需开启）
- 新增 /api/curate/stream：按步骤 emit 事件：plan → coarse → score(batch) → select → summary → explanation(field/片段)。
- 前端订阅：先渲染总结与卡片骨架；分数/精选/讲解片段逐条填充；显示每幅作品片段进度与 degraded 标记。（已验证SSE事件顺序与负载）

### 7) 失败兜底与熔断（已完成✅，已测试）
- 统一错误策略：429/5xx/timeout 触发指数退避（1s/2s/4s），仍失败则模板回退并 logged。（已实现并验证）
- 令牌桶限流（2–3 TPS）；短时 429 激增触发轻量熔断，转为模板+后台补齐，稳定后自动恢复。

### 8) 配置与观测（已完成✅，已测试）
- 环境变量：仅用 Next 默认 .env.local；浏览器可见用 NEXT_PUBLIC_*；改动后重启 dev。
- diagnostics 扩充：coarseCount、preFilterCount、scoredCount、selectedCount、glmKey、explainFromCache、explainDegraded、curveSource、providers。（已实现并验证）
